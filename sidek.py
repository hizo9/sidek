...
try:
    while True:
        ...
        if len(detected_grids) >= GRID_DETECTION_THRESHOLD:
            ...
            sensor = read_dht22()
            lux = read_light()
            lux_pct = lux_to_percentage(lux) if lux is not None else "N/A"

            temperature = sensor["temp"] if sensor else "N/A"
            humidity = sensor["humidity"] if sensor else "N/A"
            lux_val = f"{lux:.1f}" if lux is not None else "N/A"
            lux_pct_val = f"{lux_pct:.1f}" if isinstance(lux_pct, float) else "N/A"
            ...
            message = (
                ...
                f"Sunny : {lux_val} Lux, {lux_pct_val}%\n"
